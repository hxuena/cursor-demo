...
3. **生成报告初稿** —— 含执行摘要、分主题分析，以及结尾带完整 URL 的「来源」清单（正文用 `[n]` 角标引用）；
4. **结论核查（Verification）** —— 报告生成后自动进入核查阶段：先从初稿提炼出核心结论，再为**每个结论各派一个 skeptic（红队）**，提示它专门去搜索反面/矛盾证据尝试反驳。最终报告追加「结论核查」一节，每条结论标注 `[verified]` / `[disputed]`，`[disputed]` 的附上检索到的反面证据（要点 + URL）。这一步是研究流程的固定环节，不是可选项。

每一轮决策都会打印 **trace**：搜了什么词（query）、为什么搜（reason）、获得了什么（结果标题 + URL + 摘要）。核查阶段每个 skeptic 的反向检索同样会打印 trace。
...
src/agent.js    # 研究 agent：tool-use 循环、prompt caching、trace 打印，收尾调用核查
